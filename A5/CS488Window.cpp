...
//static void CS488Window::renderImGui (int framebufferWidth, int framebufferHeight);
		...
		if (key == GLFW_KEY_ESCAPE) {
			...
			if(m_paused) {
				...
				ImGui::Begin("PAUSED", &showPauseWindow, ImVec2(m_framebufferWidth*0.5,
								m_framebufferHeight*0.5), 1.0f, ImGuiWindowFlags_NoTitleBar);
				ImGui::Text("Paused");
...
void CS488Window::run (
    ...
    try {
        ...
        while (!glfwWindowShouldClose(m_window)) {
            ...
            if (this->curState == SCENE_STATE::MAINMENU) {
...
void CS488Window::mainMenuLogic() {
